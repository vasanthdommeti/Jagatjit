                ...
                <div class="col">
                    <div class="card">
                        <div class="card-header"
                            ...
                            <h5 class="card-title">Milestone Table</h5>
                            ...
                            <table id="datatable1" class="display" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->year }}</td>
                                        <td>
                                            @if (!empty($item->milestone_image))
                                            <button class="btn btn-secondary image-btn" data-bs-toggle="modal"
                                                data-bs-target="#imageModal" data-img-url="{{$item->milestone_image}}">
                                                <img src="{{$item->milestone_image}}" alt="Thumbnail"
                                                    style="width: 50px; height: 50px; border-radius: 50%;">
                                            </button>
                                            @endif
                                        </td>
                                        <td class="d-flex">
                                            <a href="{{ route('milestones.edit',[$item->id]) }}"
                                                class="btn btn-primary">Edit</a>
                                            <form action="{{ route('milestones.destroy',[$item->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
